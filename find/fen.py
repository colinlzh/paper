...
def des(s):
    if u"工程训练中心" in s:
        return s[:s.index(u"工程训练中心")+6]
    elif u"新行政楼" in s:
        return s[:s.index(u"新行政楼")+4]
    else:
        return s
if __name__ == "__main__":
    ...
    day=sc.textFile("./paper/1060all",1).filter(lambda x:u"水控" not in x and u"七宝" not in x)\
            .map(des).map(lambda x:(x.split("||")[1].split("|")[0],x)).cache()
    ...
    ac=day.map(lambda x:((x[0],x[1].split("||")[2]),1)).reduceByKey(add)\
        .map(lambda x:(x[0][1],(x[0][0],x[1]))).groupByKey()\
        .mapValues(list)\
        .mapValues(lambda x:lis2str(x,10)).flatMapValues(lambda x:x).map(lambda x:x[1]+"|"+x[0])
    # for i in ab:
    #     print(i)
    # ab.saveAsTextFile(sys.argv[1])
    ac.saveAsTextFile(sys.argv[1])
