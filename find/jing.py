...
if __name__ == "__main__":
    ...
    user=place.map(lambda x:((x.split(",")[0],x.split(",")[1].split(" ")[0]),x.split(",")[2]))\
                .filter(lambda x:u"陈瑞" in x[1])\
                .distinct()
    day=lines.mapValues(location_to_motif_anddate).mapValues(lambda x:dic2str(x))\
        .join(freq)\
        .mapValues(lambda x:motfreq(x[0],x[1]))\
        .flatMapValues(lambda x:x)
    # 11017598,2014-09-15|{0: [1], 1: [0, 2], 2: [1]}|{0: [1], 1: [0]}|26
    day=day.map(lambda x:((x[0],x[1].split("|")[0]),x[1]))\
        .join(user).map(lambda x:(x[0][1],x[1][0]))
    allnum=day.map(lambda x:(x[0],1)).reduceByKey(add)
    day=day.filter(lambda x:int(x[1].split("|")[-1])<4)\
        .mapValues(abplace)
    num=day.map(lambda x:(x[0],1)).reduceByKey(add)\
        .join(allnum).mapValues(lambda x:str(x[1])+"|"+str(x[0]))
    day=day.map(lambda x:((x[0],x[1]),1)).reduceByKey(add)\
    ...
    # act=day.map(lambda x:((x[1][0].split("|")[0],x[1][0].split("|")[3]),1))\
    #     .reduceByKey(add).filter(lambda x:int(x[0][1])<3).map(lambda x:(x[0][0],x[1]))\
    #     .reduceByKey(add)\
    #     .map(lambda x:x[0]+"|"+str(x[1]))
    # point=day.map(lambda x:x[0]).mapValues(abplace).map(lambda x:(x[0]))groupByKey()\
    #         .mapValues()
